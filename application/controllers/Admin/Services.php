...
class Services extends CI_Controller {
	...
	{       
                $alldata        = $this->ad_model->getAll('mst_client','Id ASC',array('type'=>'2'));
                $data['data']   = $alldata;
		fortable('admin/service_view', $data); 
	}
        public function Add(){
            $data   = array();
            fortable('admin/service_add', $data);  
        }
        public function client_id(){
            $id             = $this->input->get('id');
            $alldata        = $this->ad_model->getSingle('mst_client',array('id'=>$id));
            $data['data']   = $alldata;
            fortable('admin/service_up', $data);
        }
